...
export default function ThemedFocusDropdown() {
  ...
  const options = useMemo(() => (["crm", "tee-sheet", "ai-ml", "web"] as FocusKey[]), []);
  const buttonRef = useRef<HTMLButtonElement | null>(null);
  const [menuPos, setMenuPos] = useState<{ top: number; left: number } | null>(null);

  const currentLabel = focus ? `Focus: ${LABELS[focus]}` : "Focus: All solutions";
  ...
  useEffect(() => {
    ...
    function updatePosition() {
      ...
      setMenuPos({ top: Math.round(rect.bottom + 8), left: Math.round(rect.right) });
    }
    updatePosition();
  ...
  return (
    <div className=" hover:cursor-pointer relative">
      ...
      {open && menuPos && typeof document !== "undefined" && createPortal(
        ...
        >
          <button
            onClick={() => { clearFocus(); setOpen(false); }}
            className={`w-full hover:cursor-pointer text-left rounded-md px-3 py-2 text-sm ${!focus ? 'bg-zinc-800/50 text-white' : 'text-zinc-300 hover:bg-zinc-900'}`}
          >
            Focus: All solutions
          </button>
          {options.map((key) => (
            <button
              key={key}
              onClick={() => { setFocus(key); setOpen(false); }}
